...
export const App = () => {
  function getUserId(id: number, users: User[]) {
  ...
  const todoMapperUser = todosFromServer.map(todo => ({
    ...todo,
    user: getUserId(todo.userId, usersFromServer),
  }));

  const [todos, setTodos] = useState([...todoMapperUser]);
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (title && userSelect) {
      const newTodo: Todo = {
        ...
        completed: false,
        userId: userSelect,
        user: getUserId(userSelect, usersFromServer),
      };
