...
## Task
Implement ability to add TODOs to the `TodoList` implemented in [Static list of colors](https://github.com/mate-academy/react_static-list-of-todos)

1. Create `App` component storing `colors` array and displaying it using `TodoList`
1. Create a form to add new TODOs
    - DON'T create a separate component for a form
    - please add labels and placeholders where they are needed
    - each TODO should have a `title` and `userId` selected from a list of given `goods` (`./src/api/goods.js`)
...
1. Add validation to the form
    - add an empty option `Choose a user` to the goods select
